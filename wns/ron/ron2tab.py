...
wndata = "data/"
wnname = "Romanian Wordnet" 
wnlang = "ron"
wnurl = "http://www.racai.ro/tools/text/rowordnet-visualizer/"
wnlicense = "CC BY SA"
...
# Data is in the file  rown.xml
#
#
sys.stderr.write('Reading XML\n')
tree = ET.parse(wndata + 'rown.xml')
root = tree.getroot()
...
for synset in root.iter('SYNSET'):
    ...
    if sid.startswith('ENG30-'):
        ...
        lems = []
        for lit in synset.findall("SYNONYM/LITERAL"):
            #print lit.text
            if lit.text not in lems:
                lems.append(lit.text.replace('_', ''))
            else:
        ...
        # es = []
        # for exe in synset.findall("USAGE[@{http://www.w3.org/XML/1998/namespace}lang='ron']"):
