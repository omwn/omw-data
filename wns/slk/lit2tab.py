...
wnname = "Lithuanian  WordNet" 
wnurl = "http://korpus.juls.savba.sk/ltskwn_en.html"
...
##
## Data is in the file lt-sk-wn-2012-12-01.txt
## id pos lemmas gloss ␞ offset lf pos

f = codecs.open(os.path.join(wndata, "lt-sk-wn-2012-12-01.txt"),  "rb", "utf-8")
log = codecs.open("%s.log" % wnlang,  "w", "utf-8")

senses = set()

for l in f:
    (lit, slv, eng) = l.strip().split(u'␞')
    engs = eng.strip().split()
    of = "{}-{}".format(engs[0],engs[2])
    (num, pos, lems, gloss) = lit.split('\t')
    # if gloss:
    #     print of, gloss
